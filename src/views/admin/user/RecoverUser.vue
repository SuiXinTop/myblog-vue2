...
<script>
import { getUserException, recoverUser } from "@/assets/js/api/userManage";
...
export default {
  ...
  methods: {
    //获取该用户的粉丝列表
    getUserException() {
      getUserException(this.page.pageNum).then((res) => {
    ...
    recoverUser() {
      ...
      recoverUser(this.userIdList).then((res) => {
        let restMsg = res.data;
        if (restMsg.code === 200) {

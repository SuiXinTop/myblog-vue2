    ...
    >
      ...
      <el-table-column prop="userImg" label="头像" width="100">
        <template slot-scope="scope">
          <el-image
            class="blog-img"
            :src="scope.row.attendUser.userImg"
            :preview-src-list="[scope.row.attendUser.userImg]"
            fit="cover"
            alt
          >
      ...
      <el-table-column prop="user" label="名称" width="250">
        <template slot-scope="scope">
          <el-button
            type="text"
            icon="el-icon-user"
            @click="toZone(scope.row.attendUser.userId)"
          >
            {{ scope.row.attendUser.userName }}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column prop="userEmail" label="邮箱" width="250">
        <template slot-scope="scope">
          <label v-text="scope.row.attendUser.userEmail" />
        </template>
      </el-table-column>
      <el-table-column prop="userState" label="用户状态" width="150">
        <template slot-scope="scope">
          <label v-if="scope.row.attendUser.userState == 0">
            <el-tag effect="dark">异常</el-tag>
          </label>
          <label v-if="scope.row.attendUser.userState == 1">
...
<script>
import { deleteAttendList, getAttendList } from "@/assets/js/api/friend";
...
export default {
  ...
  methods: {
    //获取该用户的粉丝列表
    getAttendList() {
      getAttendList(getUserId(), this.page.pageNum).then((res) => {
